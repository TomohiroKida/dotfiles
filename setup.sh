...
# -----------------------------------------------------------------------------
# it ask question and execute by answer
# 
# ask_exec [question] [execute command]
# question          : string comment as a first argument
# execute command   : firing if answer is yes as a another arguments
# -----------------------------------------------------------------------------
ask_exec() {
	echo -n "$1 [Y/n]: "
	shift
...
# -----------------------------------------------------------------------------
# it make symbolic link from original file path to target path.
# if FORCE_MODE value is 0 then force making link
# if target file is existed or symbolic link 
# 
# make_symlink [origin file path] [made link file path]
# -----------------------------------------------------------------------------
make_symlink() {
	...
	if [ -e "${target}" ] ; then
        ...
        if [ $FORCE_MODE = '0' ] ; then
            ...
            ask_exec "file ${target} already exists. rename it and create symlink of new one [Y/n]?: " mv "$target" "${target}.old"; ln -s "$1" "$2"
        fi
	else
		echo "${target}"" isn't existed"
    fi
}
